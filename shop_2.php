...
<?php require 'header.php'; ?>
        ...
        <div class="container container-default custom-area">
            <div class="row flex-row-reverse">
                <div class="col-lg-9 col-12 col-custom widget-mt">
                            ...
                            <form class="d-flex flex-column w-100" action="shop_2" id="myForm">
                                <div class="form-group">
                                    ...
                                    <?php if (isset($_GET['search'])) { ?>
                                                                            <input type="text" name="search" value="<?php echo $_GET['search']; ?>" style="display:none">
                    ...
                    <?php
                    if (isset($_GET['search']) && isset($_GET['mySelect']) ) {
                        switch ($_GET['mySelect']) {
                            case 1:
                                $sql = "select * from item where keyword LIKE :name order by name";
                                break;
                            case 2:
                                $sql = "select Distinct *,sum(o.qty) from item,order_items as o where keyword like :name and id=product_id group by product_id order by sum(o.qty) desc";
                                break;
                            case 4:
                                $sql = "select * from item where keyword LIKE :name order by price";
                                break; 
                            case 5:
                                $sql = "select * from item where keyword LIKE :name order by price desc";
                                break;    
                            // Add more cases as needed
                            default:
                                // Code to execute when no cases match
                    ...
                    if (isset($_GET['text']) && isset($_GET['search'])) { preg_match_all('/DA(\d+)/', $_GET['text'], $matches);?>
                        <h4 class="h4search"> You Searched About : (<?php echo $_GET['search'] ?>) between : (<?php echo  $firstNumber = $matches[1][0]?> DA AND <?php echo  $secondNumber = $matches[1][1];?> DA) <a href="unsetget" class="minicart-btn"><i class="fa fa-times" style="display:inline"></i></a></h4>
                        
                 <?php     }else{?> 
                        <?php 
                        if (isset($_GET['search'])) { ?>
                        <h4 class="h4search">You Searched About : (<?php echo $_GET['search'] ?>)<a href="unsetget" class="minicart-btn"><i class="fa fa-times" style="display:inline"></i></a></h4>
                      <?php  }else{ ?>
                      <?php 
                        if (isset($_GET['text'])) {preg_match_all('/DA(\d+)/', $_GET['text'], $matches);?>
                        <h4 class="h4search"> Active Filtrer between : (<?php echo  $firstNumber = $matches[1][0]?> DA AND <?php echo  $secondNumber = $matches[1][1];?> DA)<a href="unsetget" class="minicart-btn"><i class="fa fa-times" style="display:inline"></i></a></h4>
                      <?php  }}}?> 
                     ...
                     <?php   if (isset($_GET['text'])) {
                            ...
                            $sql = "select * from item where price>=:first and price<:second";
                            if (isset($_GET['search'])) {
                                $sql = "select * from item where price>=:first and price<:second and keyword LIKE :name";
                                $stmt =$pdo->prepare($sql);
                                $stmt->execute(["first"=>$firstNumber,"second"=>$secondNumber,"name"=>'%'.$_GET['search'].'%']);
                            }else{
                            $stmt =$pdo->prepare($sql);
                            $stmt->execute(["first"=>$firstNumber,"second"=>$secondNumber]);
                        }}else{
                        
                        if (isset($_GET['black']) || isset($_GET['red']) || isset($_GET['blue']) || isset($_GET['gray']) || isset($_GET['white'])) {
                            $sql = "select * from item where";$cpt=0;
                            if (isset($_GET['black'])) {
                                if ($cpt>0) {
                                    $sql .= "or color = 'black'";
                                }else $sql .= " color = 'black'";
                                $cpt+=1;
                            }
                            if (isset($_GET['red'])) {
                                if ($cpt>0) {
                        ...
                        if (isset($_GET['search'])) {
                            $sql = "select * from item where keyword Like :name";
                            $stmt =$pdo->prepare($sql);
                            $stmt->execute(["name"=>'%'.$_GET['search'].'%']);
                            ...
                            if (isset($_GET['mySelect'])) {
                                if ($_GET['mySelect']==4) {
                                    ...
                                    $stmt =$pdo->prepare($sql);
                                    $stmt->execute();
                                }elseif ($_GET['mySelect']==5) {
                                    $sql = "select * from item order by price desc";
                                    $stmt =$pdo->prepare($sql);
                                    $stmt->execute();
                                }elseif ($_GET['mySelect']==1) {
                                    $sql = "select * from item order by name";
                                    $stmt =$pdo->prepare($sql);
                                    $stmt->execute();
                                }
                                elseif ($_GET['mySelect']==2) {
                                    $sql = "select Distinct *,sum(o.qty) from item,order_items as o where id=product_id group by product_id order by sum(o.qty) desc";
                                    $stmt =$pdo->prepare($sql);
                                    $stmt->execute();
                                }
                            }
                                ...
                                <div class="single-product position-relative mr-0 ml-0">
                                    ...
                                    <div class="product-content-listview">
                                        ...
                                        <div class="button-listview">
                                            <a href="addtocart?w=1&id=<?php echo $row['id']; ?>&current_url=<?php echo urlencode($_SERVER['REQUEST_URI']); ?>" class="btn product-cart button-icon flosun-button dark-btn" data-toggle="tooltip" data-placement="top" title="Add to Cart"> <span>Add to Cart</span> </a>
                        ...
                        <div class="widget_inner">
                            <div class="widget-list widget-mb-1">
                                ...
                                <div class="search-box">
                                    <form action="shop_2">
                                       ...
                                       <?php if (isset($_GET['mySelect'])) { ?>
                                        <input type="text" name="mySelect" value="<?php echo $_GET['mySelect']; ?>" style="display:none">
                                        <?php } ?>
                                ...
                                <nav>
                                    <ul class="mobile-menu p-0 m-0">
                                        ...
                                        <li class="menu-item-has-children"><a href="#">MONITORs</a>
                                            <ul class="dropdown">
                                                <li><a href="shop_2?search=samsung">SAMSQUNG</a></li>
                                                <li><a href="shop_2?search=hp">HP</a></li>
                                                <li><a href="shop_2?search=lenovo">Lenovo</a></li>
                                                <li><a href="shop_2?search=asus">ASUS</a></li>
                                                <li><a href="shop_2?search=acer">ACER</a></li>
                            ...
                            <div class="widget-list widget-mb-2">
                                ...
                                <div class="sidebar-body">
                                    ...
                                    <?php if (isset($_GET['search'])) { ?>
                                        <input type="text" name="search" value="<?php echo $_GET['search']; ?>" style="display:none">
                                  <?php  } ?>                                    <button type="submit">Filter</button>
                                    </form>
                                </div>
                            </div>
   ...
   <?php if (isset($_GET['text'])) {
                            $string = "DA100 - DA136300";
                            preg_match_all('/DA(\d+)/', $_GET['text'], $matches);
                            echo  $firstNumber = $matches[1][0];
                            echo '              ';
                            echo $secondNumber = $matches[1][1];
                            $sql = "select * from item where price>:first and price<:second";
                            if (isset($_GET['search'])) {
                                $sql = "select * from item where price>:first and price<:second and keyword LIKE :name";
                                $stmt =$pdo->prepare($sql);
